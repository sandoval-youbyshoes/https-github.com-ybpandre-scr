CALL `YouHist`.`o_add_moves_to_stock`((SELECT CURDATE()-INTERVAL 365 DAY), NULL);
...
FROM
	...
	SELECT
        ...
        COALESCE(tar.qty, NULL) as alvo_atual, tar.create_date as data_cadastro_alvo, tar.write_date as data_movimentacao_alvo,
        COALESCE(on_hand.qty, 0) as estoque_em_maos, (COALESCE(in_transit.qty, 0) + COALESCE(in_transit_purchase.qty, 0) - COALESCE(et2.quantidade, 0)) as estoque_em_transito,
        COALESCE(in_transit_purchase.qty, 0) as estoque_em_transito_compra,
        ...
        ROUND(((ceiling(((2.2 * 1.06) * prod.cost_price) / 5) * 5) - 0.1),2) as preco_original,
...
VALUES 
    ('new_eval_360', now(), concat(CURRENT_USER(),"-DadosOdoo")) ON DUPLICATE KEY UPDATE 
    LUpdate=now(), 
    User=CURRENT_USER();
